...
class mysql {
    ...
    #    } # mysql::do
    #
    define do ($host = localhost, $port = 3306, $content = '', $source = '', $user = "root", $password = '', $database = '') {
        file { 
        ...
        case $password {
            '': {
                Exec["mysql-do-$name"] {
                    command +> "mysql -u$user -h$host -P3306 $database < ${mysql::snippets}/$name.sql > ${puppet::semaphores}/$name.mysql"
                } # Exec
            } # '' empty
            default: {
                Exec["mysql-do-$name"] {
                    command +> "mysql -u$user -h$host -P3306 -p$password $database < ${mysql::snippets}/$name.sql > ${puppet::semaphores}/$name.mysql",
                } # Exec
            } # default
        } # case $password
